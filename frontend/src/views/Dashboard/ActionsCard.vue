...
const submitApplication = async () => {
  try {
    const response: any = await api.applications.post(formData.value);
    if (response.success) {
      if (response.isGreater) {
        toast.success(`Your Loan is ${response.loanAmount} is greater than the average loan amount`);
      } else {
        toast.success(`Your Loan is ${response.loanAmount} is lesser than the average loan amount`);
      }
      // TODO:ENable this before pusshing
      // resetForm(); // Optionally reset the form after successful submission
    } else {
      throw new Error('Error occurred while saving application');
    }
  } catch (error) {
    toast.error(error.message);
    resetForm(); // Optionally reset the form after an error
  }
};
</script>
